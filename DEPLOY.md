...
Создайте файл `.env` в корне проекта (рядом с `docker-compose.full.yml`):
...
## 3. Обновление docker-compose для production

В `docker-compose.full.yml` пароли захардкожены. Для продакшена лучше передавать их из `.env`.

Создайте копию для продакшена или отредактируйте: замените строки с паролями на переменные. Пример для `docker-compose.full.yml`:

- В секции `postgres` → `POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-people_counter_pass}`
- В секции `backend` → `PC_DB_URL=postgresql://people_counter:${POSTGRES_PASSWORD:-people_counter_pass}@postgres:5432/people_counter_db`
- Логин/пароль камеры в `backend` и `mediamtx` — из `${PC_DAHUA_USERNAME}`, `${PC_DAHUA_PASSWORD}`

Либо используйте готовый `docker-compose.prod.yml` (см. ниже), если он добавлен в репозиторий.
